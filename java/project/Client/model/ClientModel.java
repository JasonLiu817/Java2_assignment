...
import project.Client.chatroom.MainView;
import project.Utils.GsonUtils;
import project.bean.ClientUser;
import project.bean.Message;
...
//                    user.setStatus(user.getStatus().substring(0, user.getStatus().length() - 3));
    ...
    class KeepAliveWatchDog implements Runnable {
        ...
        public void run() {
            ...
            map.put(Constants.COMMAND, Constants.COM_KEEP);
            try {
...
//                    System.out.println("500ms keep");
    ...
    private void handleMessage(String message) {
        ...
        switch (command) {
            case Constants.COM_GROUP:
                ...
                ArrayList<String> userData = (ArrayList<String>) gsonMap.get(Constants.COM_GROUP);
                ...
                Platform.runLater(new Runnable() {
                    @Override
                    public void run() {
                        System.out.println(finalOnlineUserNum);
            ...
            case Constants.COM_CHATALL:
                m = new Message();
                m.setTimer((String) gsonMap.get(Constants.TIME));
                m.setSpeaker((String) gsonMap.get(Constants.SPEAKER));
                m.setContent((String) gsonMap.get(Constants.CONTENT));
            ...
            case Constants.COM_CHATWITH:
                String speaker = (String) gsonMap.get(Constants.SPEAKER);
                String receiver = (String) gsonMap.get(Constants.RECEIVER);
                String time = (String) gsonMap.get(Constants.TIME);
                String content = (String) gsonMap.get(Constants.CONTENT);
                m = new Message();
    ...
    public boolean CheckLogin(String username, String IP, String password, StringBuffer buf, int type) {
        ...
        try {
            ...
            map = new HashMap<>();
            if (type == 0)
                map.put(Constants.COMMAND, Constants.COM_LOGIN);
            else
                map.put(Constants.COMMAND, Constants.COM_SIGNUP);
            ...
            Integer result = GsonUtils.Double2Integer((Double) map.get(Constants.COM_RESULT));
            if (result == Constants.SUCCESS) {
                ...
                String description = (String) map.get(Constants.COM_DESCRIPTION);
